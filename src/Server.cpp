            ...
            namespace websocket
            {
                    ...
                    const YAML::Node &configuration)
                {
                    ...
                    throw std::runtime_error(
                        "[is::sh::WebSocket::Server] Unrecognized file format type: " + value + ". Only [" + YamlFormatPemValue + "] and [" + YamlFormatAsn1Value + "] formats are supported.");
                }
                ...
                public:
                    Server()
                        : Endpoint("is::sh::WebSocket::Server"), _opened_tls_conn_counter(0), _opened_tcp_conn_counter(0)
                    {
                        // Do nothing
                    }
